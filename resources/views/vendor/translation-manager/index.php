        ...
        (function (e, t) {
            ...
            e.rails = n = {
                ...
                handleMethod: function (r) {
                    var i = n.href(r), s = r.data("method"),
                        o = r.attr("target"),
...
<div class="container-fluid">
    ...
    <div class="alert alert-success success-import" style="display:none;">
        <p>Done importing, processed <strong class="counter">N</strong> items!
            Reload this page to refresh the groups!</p>
    </div>
    <div class="alert alert-success success-find" style="display:none;">
        <p>Done searching for translations, found <strong
                    class="counter">N</strong> items!</p>
    </div>
    <div class="alert alert-success success-publish" style="display:none;">
        <p>Done publishing the translations for group '<?php echo $group ?>
            '!</p>
    </div>
    ...
    <p>
        <?php if ( ! isset($group)) : ?>
    <form hidden class="form-import" method="POST"
    ...
    <form hidden class="form-find" method="POST"
...
<?php endif; ?>
    <?php if (isset($group)) : ?>
        <form class="form-inline form-publish" method="POST"
              action="<?php echo action('\Barryvdh\TranslationManager\Controller@postPublish',
                  $group) ?>" data-remote="true" role="form"
    ...
    <form role="form" method="POST"
        ...
        <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">
        <div class="form-group">
            <p>Choose a group to display the group translations.
                <!--                If no groups-->
                <!--                are visisble, make sure you have run the migrations and imported-->
                <!--                the translations.-->
            </p>

            <select name="group" id="group" class="form-control group-select">
                <?php foreach ($groups as $key => $value): ?>
                    <option value="<?php echo $key ?>"<?php echo $key == $group
                        ? ' selected' : '' ?>><?php echo $value ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <a href="/admin" role="button" class="btn btn-primary">Back to Admin
            Panel</a>

        <div class="form-group" hidden>
            <label>Enter a new group name and start edit translations in that
                group</label>
    ...
    <?php if ($group): ?>
        <h3 class="text-danger">Press Publish button after finish</h3>
        <form hidden
              action="<?php echo action('\Barryvdh\TranslationManager\Controller@postAdd',
                  array($group)) ?>" method="POST" role="form">
            <input type="hidden" name="_token"
                   value="<?php echo csrf_token(); ?>">
            <div class="form-group">
        ...
        <table class="table">
            ...
            <?php foreach ($translations as $key => $translation): ?>
                <tr id="<?php echo $key ?>">
                    <td><?php echo $key ?></td>
                    <?php foreach ($locales as $locale): ?>
                        <?php $t = isset($translation[$locale])
                            ? $translation[$locale] : null ?>

                        <td>
                            <a href="#edit"
                               class="editable status-<?php echo $t ? $t->status
                                   : 0 ?> locale-<?php echo $locale ?>"
                               data-locale="<?php echo $locale ?>"
                               data-name="<?php echo $locale . "|" . $key ?>"
                               id="username" data-type="textarea"
                               data-pk="<?php echo $t ? $t->id : 0 ?>"
                               data-url="<?php echo $editUrl ?>"
                               data-title="Enter translation"><?php echo $t
                                    ? htmlentities($t->value, ENT_QUOTES,
                                        'UTF-8', false) : '' ?></a>
                        </td>
                    <?php endforeach; ?>
                    <?php if ($deleteEnabled): ?>
    ...
    <?php else: ?>
        <fieldset>
            ...
            <form hidden class="form-add-locale" method="POST" role="form"
